...
The vision for elect-rix.tech is still taking shape. What isn't uncertain is the stack. Every piece of this site is built on infrastructure that doesn't require a monthly SaaS subscription to function. The content pipeline is local. The build pipeline is local. The only thing in the cloud is the edge — and that's commodity infrastructure, not lock-in.
